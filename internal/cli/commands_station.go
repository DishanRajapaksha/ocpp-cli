...
func (a *App) bootNotification(args []string) error {
	...
	if fs.NArg() != 0 { return fmt.Errorf("%w: boot-notification does not accept positional arguments", config.ErrConfig) }
	cfg, format, err := resolveCommon(fs, common)
	if err != nil { return err }
	if !wasSet(fs, "model") { *model = cfg.ChargePointModel }
	if !wasSet(fs, "vendor") { *vendor = cfg.ChargePointVendor }
	if !wasSet(fs, "firmware-version") { *firmware = cfg.FirmwareVersion }
	if !wasSet(fs, "serial-number") { *serial = cfg.SerialNumber }
	if !wasSet(fs, "meter-serial-number") { *meterSerial = cfg.MeterSerialNumber }
	if !wasSet(fs, "meter-type") { *meterType = cfg.MeterType }
	if *model == "" || len(*model) > 20 { return fmt.Errorf("%w: model is required and must not exceed 20 characters", config.ErrConfig) }
	if *vendor == "" || len(*vendor) > 20 { return fmt.Errorf("%w: vendor is required and must not exceed 20 characters", config.ErrConfig) }
	if len(*firmware) > 50 || len(*serial) > 25 || len(*meterSerial) > 25 || len(*meterType) > 25 { return fmt.Errorf("%w: boot metadata exceeds OCPP 1.6 field limits", config.ErrConfig) }
...
func (a *App) heartbeat(args []string) error {
	...
	addCommonFlags(fs, &common)
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: heartbeat does not accept positional arguments", config.ErrConfig) }
	cfg, format, err := resolveCommon(fs, common)
	if err != nil { return err }
	station, ctx, cancel, err := a.connect(cfg)
	if err != nil { return err }
...
func (a *App) authorize(args []string) error {
	...
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: authorize does not accept positional arguments", config.ErrConfig) }
	...
	if operationErr != nil && !errors.Is(operationErr, ocppclient.ErrRejected) { return operationErr }
	expiry := ""
	if result.ExpiryDate != nil { expiry = formatTime(*result.ExpiryDate) }
	renderErr := renderSnapshot(a.out, format, keyValueSnapshot(result,
...
func (a *App) statusNotification(args []string) error {
	...
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: status-notification does not accept positional arguments", config.ErrConfig) }
	if *connector < 0 { return fmt.Errorf("%w: connector must be zero or greater", config.ErrConfig) }
	if !validStatus(*status) { return fmt.Errorf("%w: invalid status %q", config.ErrConfig, *status) }
	if !validErrorCode(*errorCode) { return fmt.Errorf("%w: invalid error-code %q", config.ErrConfig, *errorCode) }
	if len(*info) > 50 || len(*vendorID) > 255 || len(*vendorError) > 50 { return fmt.Errorf("%w: status metadata exceeds OCPP 1.6 field limits", config.ErrConfig) }
