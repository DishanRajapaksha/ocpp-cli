...
func LoadClientConfigForProfile(path, profile string) (ClientConfig, error) {
	...
	if len(file.Profiles) > 0 {
		...
		if err := applySettings(&cfg, profileSettings); err != nil {
			return cfg, fmt.Errorf("%w: profile %q: %v", ErrConfig, selected, err)
		}
	}
...
func applySettings(cfg *ClientConfig, s settings) error {
	if s.CentralSystemURL != "" {
		cfg.CentralSystemURL = s.CentralSystemURL
	}
	if s.ChargePointID != "" {
		cfg.ChargePointID = s.ChargePointID
	}
	if s.Username != "" {
	...
	if s.ClientCertFile != "" {
		cfg.ClientCertFile = s.ClientCertFile
	}
	if s.ClientKeyFile != "" {
		cfg.ClientKeyFile = s.ClientKeyFile
	}
	var err error
	if s.CACertBase64 != "" {
		cfg.CACertPEM, err = decodeBase64(s.CACertBase64)
		if err != nil {
			return fmt.Errorf("%w: parse ca_cert_base64: %v", ErrConfig, err)
		}
	}
	if s.ClientCertBase64 != "" {
		cfg.ClientCertPEM, err = decodeBase64(s.ClientCertBase64)
		if err != nil {
			return fmt.Errorf("%w: parse client_cert_base64: %v", ErrConfig, err)
		}
	}
	if s.ClientKeyBase64 != "" {
		cfg.ClientKeyPEM, err = decodeBase64(s.ClientKeyBase64)
		if err != nil {
			return fmt.Errorf("%w: parse client_key_base64: %v", ErrConfig, err)
		}
	}
	if s.TLSServerName != "" {
		cfg.TLSServerName = s.TLSServerName
	}
	if s.InsecureSkipVerify != nil {
		cfg.InsecureSkipVerify = *s.InsecureSkipVerify
	}
	if s.Timeout != "" {
		cfg.Timeout, err = time.ParseDuration(s.Timeout)
		if err != nil {
			return fmt.Errorf("%w: parse timeout %q: %v", ErrConfig, s.Timeout, err)
		}
	}
	if s.Format != "" {
		cfg.Format = s.Format
	...
	if s.ChargePointModel != "" {
		cfg.ChargePointModel = s.ChargePointModel
	}
	if s.ChargePointVendor != "" {
		cfg.ChargePointVendor = s.ChargePointVendor
	}
	if s.FirmwareVersion != "" {
		cfg.FirmwareVersion = s.FirmwareVersion
	}
	if s.SerialNumber != "" {
		cfg.SerialNumber = s.SerialNumber
	}
	if s.MeterSerialNumber != "" {
		cfg.MeterSerialNumber = s.MeterSerialNumber
	}
	if s.MeterType != "" {
		cfg.MeterType = s.MeterType
	}
	return nil
}

func ValidateClientConfig(cfg ClientConfig) error {
	if strings.TrimSpace(cfg.CentralSystemURL) == "" {
		return fmt.Errorf("%w: central_system_url cannot be empty", ErrConfig)
	}
	u, err := url.Parse(cfg.CentralSystemURL)
	if err != nil || (u.Scheme != "ws" && u.Scheme != "wss") || u.Host == "" {
		return fmt.Errorf("%w: central_system_url must be an absolute ws:// or wss:// URL", ErrConfig)
	}
	if strings.TrimSpace(cfg.ChargePointID) == "" {
		return fmt.Errorf("%w: charge_point_id cannot be empty", ErrConfig)
	}
	if strings.Contains(cfg.ChargePointID, "/") {
		return fmt.Errorf("%w: charge_point_id cannot contain '/'", ErrConfig)
	}
	if cfg.Timeout <= 0 {
		return fmt.Errorf("%w: timeout must be greater than zero", ErrConfig)
	}
	if (cfg.Username == "") != (cfg.Password == "") {
		return fmt.Errorf("%w: username and password must be configured together", ErrConfig)
	}
	if (cfg.ClientCertFile == "") != (cfg.ClientKeyFile == "") {
		return fmt.Errorf("%w: client_cert and client_key must be configured together", ErrConfig)
	}
	if (len(cfg.ClientCertPEM) == 0) != (len(cfg.ClientKeyPEM) == 0) {
		return fmt.Errorf("%w: client_cert_base64 and client_key_base64 must be configured together", ErrConfig)
	}
	if cfg.ClientCertFile != "" && len(cfg.ClientCertPEM) > 0 {
		return fmt.Errorf("%w: choose file-based or base64 client certificates, not both", ErrConfig)
	}
	if cfg.ChargePointModel == "" || len(cfg.ChargePointModel) > 20 {
		return fmt.Errorf("%w: charge_point_model must contain 1 to 20 characters", ErrConfig)
	}
	if cfg.ChargePointVendor == "" || len(cfg.ChargePointVendor) > 20 {
		return fmt.Errorf("%w: charge_point_vendor must contain 1 to 20 characters", ErrConfig)
	}
	return nil
}
