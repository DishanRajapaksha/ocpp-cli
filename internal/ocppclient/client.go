...
func New(cfg config.ClientConfig) (Station, error) {
	if err := config.ValidateClientConfig(cfg); err != nil { return nil, fmt.Errorf("%w: %v", ErrValidation, err) }
...
func newWebSocketClient(cfg config.ClientConfig) (ws.Client, error) {
	...
	if cfg.Username != "" { client.SetBasicAuth(cfg.Username, cfg.Password) }
	return client, nil
}
...
func buildTLSConfig(cfg config.ClientConfig) (*tls.Config, error) {
	...
	if cfg.CACertFile != "" {
		...
		if err != nil { return nil, fmt.Errorf("%w: read CA certificate: %v", ErrAuthSecurity, err) }
	}
	if len(caPEM) > 0 && !roots.AppendCertsFromPEM(caPEM) { return nil, fmt.Errorf("%w: CA certificate does not contain valid PEM certificates", ErrAuthSecurity) }
	var certificates []tls.Certificate
	if cfg.ClientCertFile != "" {
		certificate, err := tls.LoadX509KeyPair(cfg.ClientCertFile, cfg.ClientKeyFile)
		if err != nil { return nil, fmt.Errorf("%w: load client certificate: %v", ErrAuthSecurity, err) }
		certificates = append(certificates, certificate)
	} else if len(cfg.ClientCertPEM) > 0 {
		certificate, err := tls.X509KeyPair(cfg.ClientCertPEM, cfg.ClientKeyPEM)
		if err != nil { return nil, fmt.Errorf("%w: parse inline client certificate: %v", ErrAuthSecurity, err) }
		certificates = append(certificates, certificate)
	}
...
func (c *client) Connect(ctx context.Context) error {
	select { case <-ctx.Done(): return fmt.Errorf("%w: %v", ErrTimeout, ctx.Err()); default: }
	if err := c.cp.Start(strings.TrimRight(c.cfg.CentralSystemURL, "/")); err != nil { return classify(err, ErrConnection) }
	return nil
}

func (c *client) Close() {
	c.closeOnce.Do(func() { if c.cp.IsConnected() { c.cp.Stop() } })
}
