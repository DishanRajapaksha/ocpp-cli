...
func (a *App) initConfig(args []string) error {
	...
	if fs.NArg() != 0 { return fmt.Errorf("%w: init-config does not accept positional arguments", config.ErrConfig) }
	flags := os.O_WRONLY | os.O_CREATE
	if *force { flags |= os.O_TRUNC } else { flags |= os.O_EXCL }
	file, err := os.OpenFile(*outputPath, flags, 0o600)
	if err != nil {
		if errors.Is(err, os.ErrExist) { return fmt.Errorf("%w: %s already exists; use --force to overwrite it", config.ErrConfig, *outputPath) }
		return fmt.Errorf("%w: create %s: %v", config.ErrConfig, *outputPath, err)
	}
	defer file.Close()
	if _, err := file.Write(config.StarterConfigYAML()); err != nil { return fmt.Errorf("write %s: %w", *outputPath, err) }
...
func (a *App) validateConfig(args []string) error {
	...
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: validate-config does not accept positional arguments", config.ErrConfig) }
...
func (a *App) testConnection(args []string) error {
	...
	addCommonFlags(fs, &common)
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: test-connection does not accept positional arguments", config.ErrConfig) }
	cfg, format, err := resolveCommon(fs, common)
	if err != nil { return err }
