...
func (a *App) meterValues(args []string) error {
	...
	if fs.NArg() != 0 { return fmt.Errorf("%w: meter-values does not accept positional arguments", config.ErrConfig) }
	if *connector < 0 { return fmt.Errorf("%w: connector must be zero or greater", config.ErrConfig) }
	if *value == "" { return fmt.Errorf("%w: value is required", config.ErrConfig) }
	if !validMeasurand(*measurand) || !validUnit(*unit) || !validReadingContext(*readingContext) || !validLocation(*location) || !validPhase(*phase) { return fmt.Errorf("%w: invalid meter-value metadata", config.ErrConfig) }
	ts, err := parseTimestamp(*timestamp)
	if err != nil { return err }
	cfg, format, err := resolveCommon(fs, common)
	if err != nil { return err }
	var txID *int
	if *transactionID >= 0 { txID = transactionID }
...
func (a *App) startTransaction(args []string) error {
	...
	timestamp := fs.String("timestamp", "now", "RFC3339 timestamp or now")
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: start-transaction does not accept positional arguments", config.ErrConfig) }
	if *connector <= 0 { return fmt.Errorf("%w: connector must be greater than zero", config.ErrConfig) }
	if *idTag == "" || len(*idTag) > 20 { return fmt.Errorf("%w: id-tag is required and must not exceed 20 characters", config.ErrConfig) }
	if *meterStart < 0 { return fmt.Errorf("%w: meter-start is required and must be zero or greater", config.ErrConfig) }
	ts, err := parseTimestamp(*timestamp)
	if err != nil { return err }
	cfg, format, err := resolveCommon(fs, common)
	if err != nil { return err }
	var reservation *int
	if *reservationID >= 0 { reservation = reservationID }
	station, ctx, cancel, err := a.connect(cfg)
	if err != nil { return err }
	defer cancel()
	defer station.Close()
	result, operationErr := station.StartTransaction(ctx, ocppclient.StartTransactionRequest{ConnectorID: *connector, IDTag: *idTag, MeterStart: *meterStart, ReservationID: reservation, Timestamp: ts})
	if operationErr != nil && !errors.Is(operationErr, ocppclient.ErrRejected) { return operationErr }
	expiry := ""
	if result.Authorization.ExpiryDate != nil { expiry = formatTime(*result.Authorization.ExpiryDate) }
	...
	if renderErr != nil { return renderErr }
	return operationErr
}
...
func (a *App) stopTransaction(args []string) error {
	...
	timestamp := fs.String("timestamp", "now", "RFC3339 timestamp or now")
	if err := fs.Parse(args); err != nil { return err }
	if fs.NArg() != 0 { return fmt.Errorf("%w: stop-transaction does not accept positional arguments", config.ErrConfig) }
	if *transactionID < 0 { return fmt.Errorf("%w: transaction-id is required and must be zero or greater", config.ErrConfig) }
	if *meterStop < 0 { return fmt.Errorf("%w: meter-stop is required and must be zero or greater", config.ErrConfig) }
	if len(*idTag) > 20 { return fmt.Errorf("%w: id-tag must not exceed 20 characters", config.ErrConfig) }
	if !validStopReason(*reason) { return fmt.Errorf("%w: invalid reason %q", config.ErrConfig, *reason) }
	ts, err := parseTimestamp(*timestamp)
	if err != nil { return err }
	cfg, format, err := resolveCommon(fs, common)
	if err != nil { return err }
	...
	defer cancel()
	defer station.Close()
	result, operationErr := station.StopTransaction(ctx, ocppclient.StopTransactionRequest{TransactionID: *transactionID, MeterStop: *meterStop, IDTag: *idTag, Reason: *reason, Timestamp: ts})
	if operationErr != nil && !errors.Is(operationErr, ocppclient.ErrRejected) { return operationErr }
	status := ""
	if result.Authorization != nil { status = result.Authorization.Status }
	renderErr := renderSnapshot(a.out, format, keyValueSnapshot(result,
